...
- **Mục tiêu Use Case:**
  1. Kích hoạt Agent chạy ngầm  để truy vết dấu vết số  của ứng viên trên Internet (Google Search, GitHub, LinkedIn, Facebook).
  2. Tự động đối chiếu chéo dữ liệu tìm được với thông tin trong CV  nhằm phát hiện sự gian dối, không trung thực hoặc sai lệch về:
     - Lịch sử thời gian làm việc giữa CV và LinkedIn.
     - Năng lực, dự án thực tế và đóng góp công nghệ giữa CV và kho mã nguồn / lượt đóng góp GitHub.
  3. Xuất Báo cáo sai lệch  kèm bằng chứng liên kết.
  4. Cho phép chuyên viên dán link thủ công , xem xét báo cáo và cập nhật trạng thái hồ sơ: *"Xác thực uy tín"*, *"Gắn cờ rủi ro"*, hoặc *"Không tìm thấy"*.
...
- **Luồng chính:**
  1. Chuyên viên tuyển dụng nhấn nút "Kiểm chứng" tại giao diện Chi tiết hồ sơ ứng viên.
  2. Hệ thống hiển thị trạng thái *"Đang kiểm tra..."* và kích hoạt tiến trình chạy ngầm.
  3. Agent mở các tab ẩn (các thẻ ẩn) hoặc gửi API request:
     - Tự động tạo câu truy vấn tìm kiếm  dựa trên: `Email`, `Số điện thoại`, `Họ tên + Công ty gần nhất`, `Họ tên + Trường đại học`.
     - Quét dữ liệu trên các nền tảng mục tiêu: Google Search, GitHub (cho vị trí lập trình viên), LinkedIn, Facebook.
  4. Agent thu thập dữ liệu trả về và chạy thuật toán đối chiếu chéo :
  ...
  5. Hệ thống hiển thị **Báo cáo kết quả kiểm chứng** trên giao diện:
  ...
  6. Chuyên viên tuyển dụng xem xét báo cáo và chọn hành động xác nhận:
     - Chọn *"Xác thực uy tín"* (Xác nhận uy tín).
     - Hoặc chọn *"Gắn cờ rủi ro"* (Gắn cờ rủi ro) nếu phát hiện sai lệch lớn hoặc có dấu hiệu làm đẹp CV quá mức.
...
- **Luồng thay thế:**
  ...
  - **5a (Không tìm thấy dữ liệu):** Sau khi quét, Agent không tìm thấy bất kỳ thông tin nào trùng khớp $\rightarrow$ Hiển thị thông báo: *"Không tìm thấy dấu vết số công khai của ứng viên này"* $\rightarrow$ Cho phép người dùng chọn *"Bỏ qua"* hoặc *"Thử lại với từ khóa khác"*.
- **Các luồng ngoại lệ:**
...
| :--- | :--- | :--- |
| **Thông tin định danh trong CV** | - `EP_ID1`: Đầy đủ cả Email, SĐT và Họ tên<br>- `EP_ID2`: Có Email + Họ tên (thiếu SĐT)<br>- `EP_ID3`: Có SĐT + Họ tên (thiếu Email) | - `EP_ID4`: Thiếu cả Email VÀ SĐT (Kích hoạt `EX_03`)<br>- `EP_ID5`: Email sai định dạng (ví dụ: `abc@`)<br>- `EP_ID6`: SĐT sai định dạng (chứa chữ cái hoặc ký tự lạ) |
| **Kết quả tìm kiếm dấu vết số** | - `EP_SR1`: Tìm thấy đúng profile LinkedIn và GitHub<br>- `EP_SR2`: Chỉ tìm thấy LinkedIn (hoặc chỉ GitHub)<br>- `EP_SR3`: Không tìm thấy bất kỳ dấu vết nào (Kích hoạt `5a`) | - `EP_SR4`: Bị chặn bởi Captcha/Cloudflare (Kích hoạt `EX_01`)<br>- `EP_SR5`: Trả về kết quả của người khác trùng tên (Sai lệch định danh) |
| **Mức độ sai lệch thời gian** | - `EP_TL1`: Khớp hoàn toàn thời gian (0 tháng lệch)<br>- `EP_TL2`: Lệch nhỏ trong mức chấp nhận ($< 3$ tháng)<br>- `EP_TL3`: Sai lệch lớn ($\ge 6$ tháng hoặc trùng lặp thời gian) | - `EP_TL4`: Dữ liệu ngày tháng không hợp lệ (ví dụ ngày bắt đầu sau ngày kết thúc) |
| **Nhập liên kết thủ công** | - `EP_URL1`: URL LinkedIn hợp lệ (`https://linkedin.com/in/...`)<br>- `EP_URL2`: URL GitHub hợp lệ (`https://github.com/...`) | - `EP_URL3`: URL không đúng định dạng hoặc sai domain (ví dụ dán link Youtube)<br>- `EP_URL4`: Link chết / 404 Not Found<br>- `EP_URL5`: Link chứa mã độc `javascript:alert(1)` |
| **Trạng thái sau kiểm chứng** | - `EP_ST1`: "Đã xác thực"<br>- `EP_ST2`: "Có rủi ro"<br>- `EP_ST3`: "Không tìm thấy dữ liệu" | - `EP_ST4`: Trạng thái không tồn tại trong hệ thống |
...
```
Độ lệch thời gian kinh nghiệm giữa CV và LinkedIn:
...
| | Vượt biên cảnh báo | `7 tháng` / `12 tháng` / `24 tháng` | Cảnh báo sai lệch nghiêm trọng  |
...
| Điều kiện | Rule 1 | Rule 2 | Rule 3 | Rule 4 | Rule 5 | Rule 6 | Rule 7 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
...
| **A2: Hiển thị Báo cáo: Cảnh báo sai lệch** | | **X** | | | | | |
| **A3: Cập nhật trạng thái "Đã xác thực"** | **X** | | | | | | |
| **A4: Cập nhật trạng thái "Có rủi ro"** | | **X** | | | | | |
...
| Yêu cầu kiểm thử Use Case 08 | Mã ca kiểm thử | Mức độ ưu tiên |
| :--- | :--- | :---: |
| **Luồng chính: Khởi chạy Agent & Quét ngầm** | `TC_UC08_001`, `TC_UC08_002`, `TC_UC08_003` | P1 - Rất cao |
| **Luồng chính: Đối chiếu thời gian & kỹ năng** | `TC_UC08_004`, `TC_UC08_005`, `TC_UC08_006` | P1 - Cao |
| **Luồng chính: Báo cáo & Cập nhật trạng thái** | `TC_UC08_007`, `TC_UC08_008`, `TC_UC08_009` | P1 - Cao |
| **Luồng thay thế 3a: Nhập link thủ công** | `TC_UC08_010`, `TC_UC08_011`, `TC_UC08_012` | P2 - Trung bình |
| **Luồng thay thế 5a: Không tìm thấy dữ liệu số** | `TC_UC08_013`, `TC_UC08_014`, `TC_UC08_015` | P2 - Trung bình |
| **Ngoại lệ EX_01: Bị chặn bởi Captcha** | `TC_UC08_016`, `TC_UC08_017` | P1 - Cao |
| **Ngoại lệ EX_02: Mất kết nối Internet** | `TC_UC08_018`, `TC_UC08_019` | P1 - Cao |
| **Ngoại lệ EX_03: Hồ sơ thiếu thông tin định danh**| `TC_UC08_020`, `TC_UC08_021`, `TC_UC08_022` | P1 - Rất cao |
| **Phân tích giá trị biên Sai lệch thời gian** | `TC_UC08_023`, `TC_UC08_024`, `TC_UC08_025`, `TC_UC08_026` | P2 - Cao |
| **Đối chiếu chéo chuyên sâu GitHub cho kỹ năng lập trình** | `TC_UC08_027`, `TC_UC08_028`, `TC_UC08_029` | P2 - Trung bình |
| **Kiểm thử Bảo mật, XSS và Xử lý đồng thời**| `TC_UC08_030`, `TC_UC08_031`, `TC_UC08_032`, `TC_UC08_033` | P1 / P2 |
| **Đoán lỗi: Tên phổ biến và Dữ liệu rác**| `TC_UC08_034`, `TC_UC08_035`, `TC_UC08_036`, `TC_UC08_037`, `TC_UC08_038` | P2 / P3 |

---

## 📑 PHẦN 5: BẢNG ĐẶC TẢ CHI TIẾT CÁC CA KIỂM THỬ

### NHÓM 1: KHỞI CHẠY AGENT VÀ QUÉT NGẦM TRÊN CÁC NỀN TẢNG
...
- **Kỹ thuật áp dụng:** Use Case Testing (Bước 1-2).
...
- **Các bước thực hiện:**
  1. Tại giao diện Chi tiết hồ sơ, nhấn nút "Kiểm chứng".
...
- **Kết quả mong đợi:**
  ...
  - Giao diện không bị đóng băng giao diện không bị khóa, người dùng vẫn có thể cuộn xem trang.

---

#### `TC_UC08_002` - Tự động tạo câu truy vấn tìm kiếm thông minh
- **Mục đích:** Đảm bảo Agent kết hợp các trường dữ liệu để tạo truy vấn chính xác, tránh nhầm lẫn người trùng tên.
- **Loại kiểm thử:** Kiểm thử logic nghiệp vụ.
...
#### `TC_UC08_003` - Quét thành công hồ sơ khi chỉ có Email
...
### NHÓM 2: ĐỐI CHIẾU THỜI GIAN VÀ NĂNG LỰC

#### `TC_UC08_004` - Đối chiếu thời gian làm việc hoàn toàn trùng khớp
...
- **Loại kiểm thử:** Kiểm thử xác thực kỹ năng.
...
### NHÓM 3: BÁO CÁO KIỂM CHỨNG VÀ CẬP NHẬT TRẠNG THÁI

#### `TC_UC08_007` - Hiển thị đầy đủ Báo cáo kết quả kiểm chứng
...
- **Kết quả mong đợi:**
  - Báo cáo ghim vào hồ sơ ứng viên gồm:
    ...
    2. Bảng đối chiếu thời gian làm việc giữa CV và LinkedIn.
    3. Bảng đối chiếu kỹ năng giữa CV và GitHub.
    4. Khối cảnh báo sai lệch có màu phân cấp (Xanh: Khớp, Vàng: Lệch nhẹ, Đỏ: Sai lệch lớn).

---

#### `TC_UC08_008` - Chọn hành động "Xác thực uy tín"
...
- **Kết quả mong đợi:**
  - Hệ thống cập nhật trạng thái kiểm chứng của hồ sơ thành: *"Đã xác thực"* kèm biểu tượng tích xanh uy tín.
  - Lưu nhật ký kiểm tra gồm: Người duyệt, thời gian duyệt, các link mạng xã hội đã tìm thấy.
  - Hiển thị thông báo: *"Cập nhật trạng thái kiểm chứng thành công"*.

---

#### `TC_UC08_009` - Chọn hành động "Gắn cờ rủi ro"
...
- **Kết quả mong đợi:**
  - Trạng thái kiểm chứng của hồ sơ chuyển thành: *"Có rủi ro"* kèm biểu tượng cảnh báo đỏ.
...
- **Loại kiểm thử:** Kiểm thử luồng thay thế.
...
#### `TC_UC08_013` - [5a] Không tìm thấy dấu vết số công khai
...
- **Kết quả mong đợi:**
  - Trạng thái kiểm chứng giữ nguyên hoặc ghi nhận: *"Không tìm thấy"* (Không tìm thấy dữ liệu).
...
- **Loại kiểm thử:** Kiểm thử phục hồi sau ngoại lệ.
...
### NHÓM 6: PHÂN TÍCH GIÁ TRỊ BIÊN ĐỘ LỆCH THỜI GIAN

#### `TC_UC08_023` - Kiểm thử biên Độ lệch thời gian: Lệch 1 tháng
...
#### `TC_UC08_024` - Kiểm thử biên Độ lệch thời gian: Lệch cận biên cảnh báo 5 tháng
...
- **Kết quả mong đợi:** Hiển thị lưu ý thông tin màu vàng cảnh báo, chưa kích hoạt cảnh báo rủi ro nghiêm trọng .

#### `TC_UC08_025` - Kiểm thử biên Độ lệch thời gian: Đúng ngưỡng biên cảnh báo 6 tháng
...
#### `TC_UC08_026` - Kiểm thử biên Độ lệch thời gian: Trùng lặp thời gian làm việc bất khả thi
- **Mục đích:** Phát hiện ứng viên khai làm việc đồng thời toàn thời gian tại 2 công ty khác nhau.
- **Loại kiểm thử:** Kiểm thử trường hợp biên / Nghiệp vụ.
...
### NHÓM 7: ĐỐI CHIẾU CHUYÊN SÂU GITHUB
...
#### `TC_UC08_028` - Phân biệt dự án sao chép và dự án tự phát triển trên GitHub
...
- **Mục đích:** Xác minh bảng xếp hạng ngôn ngữ trên GitHub bảng xếp hạng ngôn ngữ trên GitHub có tương đồng với thế mạnh khai trong CV.
- **Loại kiểm thử:** Kiểm thử chức năng.
...
### NHÓM 8: AN TOÀN BẢO MẬT, TƯƠNG TRANH VÀ ĐOÁN LỖI
...
#### `TC_UC08_031` - Nhấn nút "Kiểm chứng" liên tục nhiều lần
- **Mục đích:** Ngăn chặn việc tạo ra hàng loạt tiến trình ngầm ngăn tạo ra nhiều tiến trình ngầm làm nghẽn tài nguyên trình duyệt và bị chặn IP.
...
- **Loại kiểm thử:** Kiểm thử kiến trúc / Độ tin cậy.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P2 - Cao.
- **Các bước thực hiện:**
  1. Nhấn "Kiểm chứng".
...
#### `TC_UC08_033` - Tránh nhận diện sai người trùng tên
- **Mục đích:** Xử lý trường hợp ứng viên có tên quá phổ biến (ví dụ: "Nguyễn Văn Tuấn", "Trần Thị Mai").
- **Loại kiểm thử:** Kiểm thử độ chính xác thuật toán.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P1 - Cao.
...
#### `TC_UC08_034` - Lưu trữ lịch sử các lần kiểm chứng
- **Mục đích:** Đảm bảo tính minh bạch, có bằng chứng phục vụ kiểm toán tuyển dụng.
- **Loại kiểm thử:** Kiểm thử tính toàn vẹn dữ liệu.
...
#### `TC_UC08_036` - Dán link LinkedIn có chứa tham số theo dõi
- **Mục đích:** Kiểm tra hệ thống tự động làm sạch URL trước khi lưu trữ chuẩn hóa đường dẫn URL.
...
#### `TC_UC08_037` - Kiểm chứng lại hồ sơ đã từng được xác thực
- **Mục đích:** Cho phép cập nhật lại dấu vết số của ứng viên sau một khoảng thời gian.
- **Loại kiểm thử:** Kiểm thử chức năng.
- **Kỹ thuật áp dụng:** State Transition.
- **Mức độ ưu tiên:** P3 - Thấp.
- **Các bước thực hiện:** Hồ sơ đã ở trạng thái "Đã xác thực", bấm nút "Kiểm chứng lại".
...
| - Kiểm thử chức năng: | 16 | 42.1% |
| - Kiểm thử phi chức năng, giá trị biên và ngoại lệ: | 14 | 36.8% |
| - Kiểm thử an toàn bảo mật: | 4 | 10.5% |
| - Kiểm thử hiệu năng và độ tin cậy: | 4 | 10.6% |
| **Phân bổ theo mức độ ưu tiên:** | | |
| - P1 - Rất cao / Cao: | 19 | 50.0% |
...
2. **Tuân thủ chặt chẽ lý thuyết môn học CSE462:** Áp dụng đầy đủ 6 kỹ thuật thiết kế kiểm thử hộp đen chuẩn ISTQB CTFL gồm: *Phân vùng tương đương*, *Phân tích giá trị biên*, *Bảng quyết định*, *Kiểm thử chuyển trạng thái*, *Kiểm thử Use Case* và *Đoán lỗi*.
3. **Tính ứng dụng thực tế cao:** Giải quyết các bài toán hóc búa của Background Agent như: chống nghẽn do spam click, đối phó Captcha bot detection, xử lý người trùng tên , và phát hiện gian lận thời gian làm việc trên CV.
