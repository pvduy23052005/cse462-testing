...
- **Mã Use Case (Use Case ID):** `UC_07`
- **Tên Use Case (Use Case Name):** Quản lý và Phân tích Hồ sơ Ứng viên (Candidate Profile Management & AI Matching Analysis)
...
- **Mục tiêu Use Case:** Cho phép Chuyên viên tuyển dụng:
  1. Xem danh sách toàn bộ ứng viên đã thu thập kèm phân trang (Pagination).
  2. Lọc và tìm kiếm ứng viên đa tiêu chí (Kỹ năng, Kinh nghiệm, Trạng thái, JD).
  3. Xem chi tiết hồ sơ ứng viên (hoặc xem nhanh qua Quick View Card).
  4. Kích hoạt AI Agent để so khớp hồ sơ ứng viên với Mô tả công việc (JD), tính điểm phù hợp (*Matching Score*), phân tích khoảng cách kỹ năng (*Gap Analysis*), cảnh báo rủi ro (*Red Flags*) và tóm tắt điểm mạnh/yếu (*Summary*).
...
- **Luồng chính (Main Flow):**
  ...
  7. Hệ thống kích hoạt AI so khớp dữ liệu ứng viên với JD đã chọn và hiển thị kết quả phân tích:
     - **Matching Score:** Điểm số phù hợp trên thang 100%.
     - **Summary:** Tóm tắt ưu/nhược điểm.
     - **Gap Analysis:** Liệt kê các kỹ năng ứng viên còn thiếu so với JD.
     - **Red Flags:** Cảnh báo rủi ro (nếu có, ví dụ nhảy việc liên tục, sai lệch thời gian).
...
- **Luồng thay thế (Alternative Flows):**
  - **3a (Reset bộ lọc / Clear Filter):** Tại bước 3, người dùng nhấn "Xóa bộ lọc" $\rightarrow$ Hệ thống hiển thị lại danh sách ứng viên ban đầu.
  - **4a (Xem nhanh / Quick View):** Người dùng rê chuột (hover) vào avatar ứng viên $\rightarrow$ Hệ thống hiển thị Quick Card tóm tắt: Tên, Chức danh hiện tại, Link CV gốc.
...
| :--- | :--- | :--- |
| **Kỹ năng cần lọc (Skills Filter)** | - `EP_SK1`: Kỹ năng đơn lẻ có trong DB (ví dụ: `ReactJS`)<br>- `EP_SK2`: Tổ hợp nhiều kỹ năng đồng thời (ví dụ: `ReactJS`, `Node.js`) | - `EP_SK3`: Kỹ năng không tồn tại trong bất kỳ hồ sơ nào<br>- `EP_SK4`: Nhập chuỗi ký tự đặc biệt, mã độc SQL/XSS (`' OR 1=1--`, `<script>`) |
| **Số năm kinh nghiệm (Exp Filter)** | - `EP_EX1`: `Exp = 0` (Fresher/Chưa có kinh nghiệm)<br>- `EP_EX2`: `0 < Exp < 2` năm (Junior)<br>- `EP_EX3`: `2 <= Exp < 5` năm (Middle)<br>- `EP_EX4`: `Exp >= 5` năm (Senior) | - `EP_EX5`: Số âm (`Exp < 0`)<br>- `EP_EX6`: Nhập chữ/ký tự không phải số vào ô kinh nghiệm |
| **Trạng thái ứng viên (Status Filter)** | - `EP_ST1`: "Mới" (New)<br>- `EP_ST2`: "Đã xem" (Viewed)<br>- `EP_ST3`: "Phù hợp" (Matched)<br>- `EP_ST4`: "Phỏng vấn" (Interview)<br>- `EP_ST5`: "Từ chối" (Rejected) | - `EP_ST6`: Giá trị trạng thái không thuộc hệ thống (giả mạo payload API) |
| **Lựa chọn JD so sánh (JD Selection)** | - `EP_JD1`: Chọn 1 JD hợp lệ đang hoạt động (Active JD) | - `EP_JD2`: Không chọn JD nào (bỏ trống)<br>- `EP_JD3`: JD đã bị xóa hoặc tạm ngưng |
| **Điểm phù hợp AI (Matching Score)** | - `EP_SC1`: Thấp ($0\% \le \text{Score} < 50\%$)<br>- `EP_SC2`: Trung bình ($50\% \le \text{Score} < 75\%$)<br>- `EP_SC3`: Cao ($75\% \le \text{Score} \le 100\%$) | - `EP_SC4`: Điểm âm ($\text{Score} < 0\%$) hoặc vượt mức ($\text{Score} > 100\%$) do lỗi thuật toán AI |
| **Phân trang (Pagination)** | - `EP_PG1`: Trang đầu tiên (Page 1)<br>- `EP_PG2`: Các trang giữa ($1 < \text{Page} < \text{Total}$)<br>- `EP_PG3`: Trang cuối cùng ($\text{Page} = \text{Total}$) | - `EP_PG4`: Nhập số trang $\le 0$<br>- `EP_PG5`: Nhập số trang vượt quá tổng số trang hiện có |

---

### 2. Kỹ thuật Phân tích giá trị biên (Boundary Value Analysis - BVA)

Áp dụng cho các tham số định lượng: **Số năm kinh nghiệm**, **Điểm số Matching Score**, **Phân trang** và **Thời gian AI phân tích**:

```
Thang điểm Matching Score (%):
...
| **Matching Score (0% - 100%)** | Biên dưới tuyệt đối | `0%` | Hiển thị mức độ không phù hợp (0%) |
...
| | Đúng 1 trang | `10 hồ sơ` | Hiển thị 1 trang, nút "Next/Trang sau" bị disabled |
...
| **A5: Hiển thị Matching Score, Gap, Summary, Red Flags** | | | | **X** | | **X** |
...
#### A. Vòng đời Trạng thái Hồ sơ Ứng viên (Candidate Profile Status):
```mermaid
stateDiagram-v2
    [*] --> Moi: Thu thập từ UC_06 (Mới)
    Moi --> DaXem: Click xem chi tiết hồ sơ
    DaXem --> PhuHop: AI Matching Score >= 75% hoặc Đánh giá Phù hợp
    DaXem --> TuChoi: Không phù hợp / Loại hồ sơ
    PhuHop --> PhongVan: Chuyển sang vòng Phỏng vấn
    PhongVan --> TrungTuyen: Đạt phỏng vấn (Hired)
    PhongVan --> TuChoi: Trượt phỏng vấn
    TrungTuyen --> [*]
    TuChoi --> [*]
```

#### B. Vòng đời Phân tích AI Matching (AI Analysis Lifecycle):
```mermaid
stateDiagram-v2
...
| Thành phần Yêu cầu Use Case 07 | Mã Test Case (Test Case IDs) | Mức độ ưu tiên |
| :--- | :--- | :---: |
| **Luồng chính: Xem danh sách & Phân trang** | `TC_UC07_001`, `TC_UC07_002`, `TC_UC07_003` | P1 (High) |
| **Luồng chính: Bộ lọc tìm kiếm đa tiêu chí** | `TC_UC07_004`, `TC_UC07_005`, `TC_UC07_006`, `TC_UC07_007` | P1 (High) |
| **Luồng chính: Xem chi tiết hồ sơ ứng viên** | `TC_UC07_008`, `TC_UC07_009` | P1 (High) |
| **Luồng chính: Kích hoạt & Hiển thị kết quả AI** | `TC_UC07_010`, `TC_UC07_011`, `TC_UC07_012`, `TC_UC07_013` | P1 (Critical) |
| **Luồng chính: Lưu kết quả & Cập nhật trạng thái** | `TC_UC07_014`, `TC_UC07_015`, `TC_UC07_016` | P1 (High) |
| **Luồng thay thế 3a: Xóa bộ lọc (Clear Filter)** | `TC_UC07_017`, `TC_UC07_018` | P2 (Medium) |
| **Luồng thay thế 4a: Xem nhanh hồ sơ (Quick View)** | `TC_UC07_019`, `TC_UC07_020` | P2 (Medium) |
| **Luồng thay thế 8a: Xuất báo cáo cá nhân PDF** | `TC_UC07_021`, `TC_UC07_022` | P2 (Medium) |
| **Ngoại lệ EX_01: Không tìm thấy kết quả lọc** | `TC_UC07_023`, `TC_UC07_024` | P1 (High) |
| **Ngoại lệ EX_02: Thiếu JD khi bấm Phân tích** | `TC_UC07_025`, `TC_UC07_026` | P1 (Critical) |
| **Phân tích giá trị biên (BVA) Kinh nghiệm & Score** | `TC_UC07_027`, `TC_UC07_028`, `TC_UC07_029`, `TC_UC07_030` | P2 (High) |
| **Điều kiện tiên quyết & Xử lý bất thường** | `TC_UC07_031`, `TC_UC07_032`, `TC_UC07_033` | P2 (Medium) |
| **Kiểm thử Bảo mật & Đoán lỗi (Security & Edge Cases)**| `TC_UC07_034`, `TC_UC07_035`, `TC_UC07_036`, `TC_UC07_037`, `TC_UC07_038` | P1 / P2 |

---

## 📑 PHẦN 5: BẢNG ĐẶC TẢ CHI TIẾT CÁC TEST CASE (TEST SUITE SPECIFICATION)

### NHÓM 1: XEM DANH SÁCH & PHÂN TRANG (PAGINATION)
...
- **Loại kiểm thử (Test Type):** Functional / UI Verification.
- **Kỹ thuật áp dụng (Technique):** Use Case Testing.
...
#### `TC_UC07_002` - Chuyển tiếp giữa các trang (Next / Previous Pagination)
...
- **Mục đích:** Kiểm tra trạng thái vô hiệu hóa (disabled) tại các điểm biên phân trang.
- **Loại kiểm thử:** Boundary Testing / UI.
...
### NHÓM 2: BỘ LỌC TÌM KIẾM ĐA TIÊU CHÍ (FILTER & SEARCH)

#### `TC_UC07_004` - Lọc ứng viên theo Kỹ năng đơn lẻ (Single Skill Filter)
...
#### `TC_UC07_005` - Lọc ứng viên theo Số năm kinh nghiệm (Experience Filter)
...
#### `TC_UC07_006` - Lọc ứng viên theo Trạng thái hồ sơ (Status Filter)
...
#### `TC_UC07_007` - Kết hợp lọc đồng thời nhiều tiêu chí (Kỹ năng + Kinh nghiệm + Trạng thái)
- **Mục đích:** Kiểm tra tính chính xác của phép toán logic `AND` khi kết hợp đa tiêu chí lọc.
- **Loại kiểm thử:** Functional / Complex Search.
...
### NHÓM 3: XEM CHI TIẾT HỒ SƠ & THAY ĐỔI TRẠNG THÁI
...
- **Kết quả mong đợi:**
  - Trạng thái của ứng viên này trong CSDL và trên giao diện tự động chuyển thành "Đã xem" (Viewed).

---

### NHÓM 4: KÍCH HOẠT & HIỂN THỊ KẾT QUẢ PHÂN TÍCH AI (AI MATCHING)
...
- **Loại kiểm thử:** Functional / AI Core Feature.
- **Kỹ thuật áp dụng:** Use Case Testing (Bước 6-7).
- **Mức độ ưu tiên:** P1 (Critical).
- **Tiền điều kiện:** Đang mở chi tiết hồ sơ ứng viên Nguyễn Văn A (Fullstack Developer); Hệ thống có sẵn JD "Senior Backend Engineer".
- **Các bước thực hiện:**
  1. Tại mục chọn JD, chọn "Senior Backend Engineer".
  2. Nhấn nút "Phân tích AI".
  3. Chờ AI xử lý trong thời gian $< 10$ giây.
- **Kết quả mong đợi:**
  - Hiển thị hiệu ứng loading (Spinner / Progress bar).
  - Sau khi xử lý xong, hiển thị đầy đủ 4 thành phần kết quả:
    1. **Matching Score:** Thang điểm % (ví dụ: 82%).
...
#### `TC_UC07_011` - Kiểm tra tính chính xác của phần Gap Analysis (Khoảng cách kỹ năng)
- **Mục đích:** Đảm bảo AI chỉ ra đúng các kỹ năng bắt buộc trong JD mà CV ứng viên chưa có.
- **Loại kiểm thử:** Functional / Business Logic.
- **Kỹ thuật áp dụng:** Use Case Testing.
- **Mức độ ưu tiên:** P2 (High).
- **Dữ liệu thử nghiệm:**
...
- **Loại kiểm thử:** Functional / Data Persistence.
...
#### `TC_UC07_015` - [3a] Xóa bộ lọc và reset danh sách ứng viên (Clear Filter)
...
- **Các bước thực hiện:**
  ...
  3. Nhấn nút "Xóa bộ lọc" (Clear Filter).
- **Kết quả mong đợi:**
...
#### `TC_UC07_016` - [4a] Xem nhanh thông tin ứng viên bằng thao tác Hover (Quick View Card)
- **Mục đích:** Xác minh tính năng xem nhanh hồ sơ mà không cần mở trang chi tiết.
- **Loại kiểm thử:** UI/UX / Functional.
- **Kỹ thuật áp dụng:** Use Case Testing (`Luồng 4a`).
- **Mức độ ưu tiên:** P2 (Medium).
- **Các bước thực hiện:**
  1. Tại bảng danh sách ứng viên, rê chuột (hover) vào hình đại diện (avatar) của ứng viên.
  2. Giữ chuột trong 0.5 giây.
- **Kết quả mong đợi:**
  - Xuất hiện popup Quick Card nổi trên màn hình.
  - Quick Card hiển thị đúng tóm tắt: Họ tên, Chức danh hiện tại, và Link liên kết đến CV gốc.
  - Khi rê chuột ra ngoài, popup tự động ẩn đi mượt mà, không bị giật lag hay treo popup.

---

#### `TC_UC07_017` - [4a] Nhấp vào Link CV gốc từ Quick Card
- **Mục đích:** Đảm bảo liên kết mở tệp CV gốc hoạt động chính xác từ thẻ Quick View.
- **Loại kiểm thử:** Functional / Link Navigation.
- **Kỹ thuật áp dụng:** Use Case Testing (`Luồng 4a`).
- **Mức độ ưu tiên:** P3 (Low).
- **Các bước thực hiện:**
  1. Rê chuột vào avatar để hiển thị Quick Card.
...
- **Loại kiểm thử:** Functional / Report Generation.
...
- **Kết quả mong đợi:**
  ...
  - Nội dung file PDF trình bày đẹp mắt, rõ ràng, gồm: Thông tin ứng viên, Tên JD so sánh, Điểm số Matching Score, Summary, Gap Analysis và Red Flags.

---

### NHÓM 6: KIỂM THỬ CÁC NGOẠI LỆ (EX_01, EX_02)

#### `TC_UC07_019` - [EX_01] Lọc không có kết quả phù hợp (Bộ lọc không thỏa mãn)
- **Mục đích:** Xác minh thông báo giao diện khi tìm kiếm/lọc không có ứng viên nào đáp ứng.
- **Loại kiểm thử:** Negative / Exception.
...
- **Loại kiểm thử:** Functional / Validation / Exception.
...
- **Kết quả mong đợi:**
  ...
  - Tự động mở rộng (focus/highlight) Dropdown danh sách JD để người dùng chọn nhanh.
...
### NHÓM 7: PHÂN TÍCH GIÁ TRỊ BIÊN (BVA) & ĐỘ ĐO ĐỊNH LƯỢNG

#### `TC_UC07_022` - BVA Số năm kinh nghiệm: Giá trị cận biên dưới (1.9 năm)
...
#### `TC_UC07_023` - BVA Số năm kinh nghiệm: Ngay tại biên chuẩn (2.0 năm)
...
#### `TC_UC07_024` - BVA Matching Score: Điểm biên tuyệt đối 0%
- **Mục đích:** Xác minh hệ thống hiển thị đúng khi hồ sơ hoàn toàn lệch chuẩn với JD (không trùng bất kỳ kỹ năng nào).
- **Loại kiểm thử:** Boundary Testing / AI Output.
- **Kỹ thuật áp dụng:** Boundary Value Analysis (Biên 0%).
- **Mức độ ưu tiên:** P2 (Medium).
- **Dữ liệu thử nghiệm:** CV Kế toán viên so khớp với JD Lập trình viên Trí tuệ nhân tạo (AI Engineer).
- **Kết quả mong đợi:**
  - Matching Score hiển thị `0%`.
  - Gap Analysis liệt kê toàn bộ kỹ năng của JD; Summary nhận xét không phù hợp.

#### `TC_UC07_025` - BVA Matching Score: Điểm biên tuyệt đối 100%
...
- **Mức độ ưu tiên:** P2 (Medium).
- **Kết quả mong đợi:** Matching Score hiển thị `100%`, Gap Analysis trống, Summary đánh giá ứng viên xuất sắc/hoàn toàn phù hợp.

#### `TC_UC07_026` - BVA Phân trang: Tổng số hồ sơ đúng bằng kích thước 1 trang (10 hồ sơ)
...
#### `TC_UC07_027` - BVA Phân trang: Tổng số hồ sơ vượt biên 1 bản ghi (11 hồ sơ)
...
### NHÓM 8: ĐIỀU KIỆN TIÊN QUYẾT & BẤT THƯỜNG DỮ LIỆU

#### `TC_UC07_028` - Kiểm tra Tiền điều kiện: CSDL hoàn toàn chưa có hồ sơ nào (0 hồ sơ)
- **Mục đích:** Đảm bảo hệ thống xử lý thân thiện khi hệ thống mới tinh chưa có dữ liệu.
- **Loại kiểm thử:** Boundary / Pre-condition Testing.
...
- **Kết quả mong đợi:**
  ...
  - Hiển thị giao diện rỗng kèm hình minh họa (Empty State Illustration) và dòng chữ: *"Chưa có hồ sơ ứng viên nào. Hãy sử dụng tính năng Quét web hoặc Upload CV để thu thập hồ sơ mới!"*.
...
- **Loại kiểm thử:** Pre-condition / Exception.
- **Kỹ thuật áp dụng:** Pre-condition Verification.
- **Mức độ ưu tiên:** P2 (Medium).
...
- **Loại kiểm thử:** Concurrency / Edge Case.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P2 (Medium).
- **Các bước thực hiện:**
...
### NHÓM 9: KIỂM THỬ BẢO MẬT, HIỆU NĂNG & ĐOÁN LỖI (SECURITY & PERFORMANCE)
...
#### `TC_UC07_033` - Nhấn nút "Phân tích AI" liên tục nhiều lần (Spam Click)
...
- **Loại kiểm thử:** Performance / Reliability.
...
#### `TC_UC07_035` - Xuất file PDF khi đang trong quá trình AI phân tích (Race Condition)
- **Mục đích:** Kiểm tra nút "In/Xuất PDF" khi dữ liệu phân tích chưa sẵn sàng.
- **Loại kiểm thử:** Concurrency / UI State.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P3 (Low).
...
#### `TC_UC07_036` - Tìm kiếm không phân biệt chữ hoa, chữ thường (Case-insensitivity)
- **Mục đích:** Đảm bảo trải nghiệm tìm kiếm tự nhiên cho chuyên viên tuyển dụng.
- **Loại kiểm thử:** Functional / Usability.
...
#### `TC_UC07_037` - Tìm kiếm kỹ năng có chứa khoảng trắng thừa (Leading / Trailing Whitespace)
...
- **Loại kiểm thử:** Security / Session Management.
...
| - Kiểm thử chức năng (Functional / Positive Testing): | 17 | 44.7% |
| - Kiểm thử phi chức năng, biên & ngoại lệ (Negative / Boundary / Exception): | 13 | 34.2% |
| - Kiểm thử an toàn bảo mật (Security / Injection / XSS / Session): | 4 | 10.5% |
| - Kiểm thử hiệu năng & độ tin cậy (Performance / Timeout / Concurrency): | 4 | 10.6% |
| **Phân bổ theo mức độ ưu tiên (Priority):** | | |
| - P1 (Critical / High): | 18 | 47.4% |
| - P2 (Medium): | 15 | 39.5% |
| - P3 (Low): | 5 | 13.1% |
...
2. **Tuân thủ chặt chẽ lý thuyết môn học CSE462:** Vận dụng đầy đủ 6 kỹ thuật thiết kế kiểm thử hộp đen chuẩn ISTQB CTFL gồm: *Phân vùng tương đương*, *Phân tích giá trị biên*, *Bảng quyết định*, *Chuyển trạng thái*, *Kiểm thử ca sử dụng* và *Đoán lỗi*.
