...
- **Mã Use Case (Use Case ID):** `UC_06`
- **Tên Use Case (Use Case Name):** Thu Thập Hồ Sơ (Candidate Profile Harvesting / Data Extraction)
...
- **Luồng chính (Main Flow - Web Parsing):**
  ...
  5. Hệ thống hiển thị Form Xem trước (*Preview Form*) với các trường đã điền sẵn.
...
- **Luồng thay thế (Alternative Flow - Upload CV File):**
  ...
  - B.5: Gửi file lên server xử lý OCR chuyển ảnh thành text, LLM bóc tách thực thể (*Entity Extraction*).
  - B.6: Chuyển về Bước 5 của Luồng chính (Hiển thị Preview Form để kiểm tra/chỉnh sửa và Lưu).
...
Nhằm đảm bảo độ bao phủ kiểm thử tối đa (*Test Coverage*) theo lý thuyết học phần CSE462, các kỹ thuật kiểm thử hộp đen được áp dụng chặt chẽ như sau:
...
| :--- | :--- | :--- |
| **Định dạng file CV (File Extension)** | - `EP_F1`: File PDF (`.pdf`)<br>- `EP_F2`: File Word (`.docx`, `.doc`)<br>- `EP_F3`: File ảnh (`.png`, `.jpg`, `.jpeg`) | - `EP_F4`: File thực thi nguy hiểm (`.exe`, `.bat`, `.sh`)<br>- `EP_F5`: File nén/tài liệu khác (`.zip`, `.rar`, `.txt`, `.xlsx`)<br>- `EP_F6`: File không có phần mở rộng |
| **Dung lượng file CV (File Size)** | - `EP_S1`: $0 < \text{Size} \le 10\text{ MB}$ (Ví dụ: 1 KB, 5 MB, 10 MB) | - `EP_S2`: $\text{Size} = 0\text{ KB}$ (File rỗng)<br>- `EP_S3`: $\text{Size} > 10\text{ MB}$ (Ví dụ: 10.1 MB, 50 MB) |
| **Nền tảng trang web (Domain URL)** | - `EP_W1`: Trang profile cá nhân trên LinkedIn<br>- `EP_W2`: Trang hồ sơ ứng viên trên TopCV<br>- `EP_W3`: Trang hồ sơ ứng viên trên VietnamWorks | - `EP_W4`: Trang web hoàn toàn không liên quan (Facebook, Youtube, VnExpress)<br>- `EP_W5`: Trang thuộc domain hỗ trợ nhưng không phải trang profile (Trang Feed, Search...) |
| **Thời gian phản hồi AI (Response Time)** | - `EP_T1`: $T \le 10\text{ giây}$ (Phân tích thành công) | - `EP_T2`: $T > 10\text{ giây}$ (Kích hoạt Timeout EX_02) |
| **Trạng thái xác thực (Auth State)** | - `EP_A1`: Đã đăng nhập Extension và Token còn hạn | - `EP_A2`: Chưa đăng nhập Extension<br>- `EP_A3`: Phiên đăng nhập hết hạn (Token expired) |
| **Kết nối Internet (Network)** | - `EP_N1`: Kết nối Internet ổn định | - `EP_N2`: Không có Internet (Offline)<br>- `EP_N3`: Mất kết nối đột ngột khi đang gửi request |
...
| Điều kiện (Conditions) | Rule 1 | Rule 2 | Rule 3 | Rule 4 | Rule 5 | Rule 6 | Rule 7 | Rule 8 |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: |
| **C1: Phương thức thu thập** | Web Scan | Web Scan | Web Scan | Upload CV | Upload CV | Upload CV | Upload CV | Any |
| **C2: Nguồn hợp lệ (Domain / File type)** | Đúng (LinkedIn/TopCV) | Đúng (LinkedIn/TopCV) | Sai (Domain lạ) | Đúng (.pdf/.docx/.img) | Sai (.exe/.zip) | Đúng (.pdf/.docx) | Đúng (.pdf/.docx) | Any |
| **C3: Dung lượng file $\le 10\text{MB}$** | N/A | N/A | N/A | Đúng ($\le 10\text{MB}$) | N/A | Sai ($> 10\text{MB}$) | Đúng ($\le 10\text{MB}$) | N/A |
| **C4: Server AI phản hồi $\le 10\text{s}$** | Có | Không (Timeout) | N/A | Có | N/A | N/A | Không (Timeout) | N/A |
...
stateDiagram-v2
    ...
    Idle --> ScanningDOM: Nhấn "Quét" (Web)
    Idle --> UploadingFile: Kéo thả / Chọn file (Upload)
...
| Yêu cầu / Luồng Use Case | Mã Test Case (Test Case IDs) | Mức độ ưu tiên |
| :--- | :--- | :---: |
| **Luồng chính: Quét Web hợp lệ** | `TC_UC06_001`, `TC_UC06_002`, `TC_UC06_003`, `TC_UC06_004` | High (P1) |
| **Luồng chính: Thao tác Form Preview** | `TC_UC06_005`, `TC_UC06_006`, `TC_UC06_007` | Medium (P2) |
| **Luồng thay thế: Upload CV hợp lệ** | `TC_UC06_008`, `TC_UC06_009`, `TC_UC06_010`, `TC_UC06_011`, `TC_UC06_012` | High (P1) |
| **Phân tích biên & Phân vùng dung lượng file** | `TC_UC06_013`, `TC_UC06_014`, `TC_UC06_015`, `TC_UC06_016`, `TC_UC06_017`, `TC_UC06_018` | High (P1) |
| **Phân vùng định dạng file (Kèm bảo mật file)** | `TC_UC06_019`, `TC_UC06_020`, `TC_UC06_021`, `TC_UC06_022` | Critical (P1) |
| **Ngoại lệ EX_01: File sai định dạng/dung lượng** | `TC_UC06_023`, `TC_UC06_024` | High (P1) |
| **Ngoại lệ EX_02: AI Timeout / Lỗi kết nối** | `TC_UC06_025`, `TC_UC06_026`, `TC_UC06_027` | High (P1) |
| **Ngoại lệ EX_03: Website không hỗ trợ** | `TC_UC06_028`, `TC_UC06_029` | High (P1) |
| **Kiểm tra Điều kiện tiên quyết & Mạng** | `TC_UC06_030`, `TC_UC06_031`, `TC_UC06_032`, `TC_UC06_033` | Medium (P2) |
| **Đoán lỗi & Bảo mật dữ liệu (Error Guessing/Security)** | `TC_UC06_034`, `TC_UC06_035`, `TC_UC06_036`, `TC_UC06_037`, `TC_UC06_038` | Medium (P2) |

---

## 📑 PHẦN 5: BẢNG ĐẶC TẢ CHI TIẾT CÁC TEST CASE (TEST SUITE SPECIFICATION)

### NHÓM 1: KIỂM THỬ LUỒNG CHÍNH (MAIN FLOW - WEB PARSING)
...
- **Loại kiểm thử (Test Type):** Functional / Positive Testing.
...
- **Dữ liệu thử nghiệm (Test Data):** Trang profile LinkedIn có đầy đủ các mục thông tin.
...
- **Loại kiểm thử:** Functional / Positive Testing.
...
- **Loại kiểm thử:** Functional / Negative.
- **Kỹ thuật áp dụng:** Use Case Testing.
...
#### `TC_UC06_007` - Nhấn nút "Lưu hồ sơ" liên tục nhiều lần (Double Click / Spam Click)
- **Mục đích:** Kiểm tra xử lý tương tranh (*Concurrency / Debouncing*) tránh tạo bản ghi trùng lặp.
- **Loại kiểm thử:** Performance / Concurrency / Edge Case.
...
### NHÓM 2: KIỂM THỬ LUỒNG THAY THẾ (ALTERNATIVE FLOW - UPLOAD CV)
...
#### `TC_UC06_011` - Tải file CV bằng thao tác kéo thả (Drag & Drop)
...
- **Kết quả mong đợi:**
  - Vùng kéo thả đổi hiệu ứng nhận diện chuột (drag-over effect).
...
- **Các bước thực hiện:**
  ...
  2. Hộp thoại File Explorer (Windows) / Finder (macOS) xuất hiện.
...
### NHÓM 3: PHÂN TÍCH GIÁ TRỊ BIÊN DUNG LƯỢNG FILE (BVA - FILE SIZE)
...
- **Loại kiểm thử:** Boundary / Negative Testing.
...
- **Kết quả mong đợi:**
  - Hệ thống chặn ngay tại bước kiểm tra kích thước file (Client-side validation).
...
### NHÓM 4: PHÂN VÙNG ĐỊNH DẠNG FILE & AN TOÀN BẢO MẬT (EP & SECURITY)
...
- **Loại kiểm thử:** Security / Negative Testing.
...
#### `TC_UC06_021` - Upload file giả mạo định dạng kép (Double Extension: `cv.pdf.exe`)
...
#### `TC_UC06_022` - Upload file không có phần mở rộng (No extension)
- **Mục đích:** Kiểm tra xử lý tên file không có dấu chấm phần mở rộng.
- **Loại kiểm thử:** Negative / Edge Case.
...
### NHÓM 5: KIỂM THỬ CÁC LUỒNG NGOẠI LỆ (EXCEPTIONS: EX_01, EX_02, EX_03)
...
- **Loại kiểm thử:** Functional / UI Text Verification.
...
- **Loại kiểm thử:** Performance / Exception Testing.
...
- **Kết quả mong đợi:**
  - Đúng thời điểm 10 giây, hệ thống ngắt request (Abort/Timeout).
...
- **Loại kiểm thử:** Functional / End-to-End.
...
#### `TC_UC06_028` - [EX_03] Nhấn nút "Quét" trên website không được hỗ trợ (Website lạ)
...
#### `TC_UC06_029` - [EX_03] Nhấn nút "Quét" trên domain LinkedIn nhưng sai trang (Trang Feed/Search)
- **Mục đích:** Kiểm tra Extension phân biệt được giữa trang profile cá nhân và các trang phụ trợ khác.
- **Loại kiểm thử:** Negative / Boundary.
...
### NHÓM 6: ĐIỀU KIỆN TIÊN QUYẾT & MẠNG (PRE-CONDITIONS & NETWORK)
...
- **Loại kiểm thử:** Security / Functional.
...
- **Kết quả mong đợi:**
  ...
  - Chuyển hướng người dùng về màn hình Đăng nhập (Login screen) kèm thông báo: *"Vui lòng đăng nhập để sử dụng tính năng này"*.

---

#### `TC_UC06_031` - Phiên đăng nhập hết hạn (Token Expired) trong lúc lưu hồ sơ
- **Mục đích:** Xử lý trường hợp JWT Token hết hạn giữa chừng.
- **Loại kiểm thử:** Security / Session Handling.
...
#### `TC_UC06_032` - Mất kết nối Internet hoàn toàn (Offline) trước khi Quét
- **Mục đích:** Kiểm tra xử lý khi ngắt kết nối mạng.
- **Loại kiểm thử:** Network / Reliability.
...
- **Loại kiểm thử:** Network / Data Integrity.
...
### NHÓM 7: KIỂM THỬ ĐOÁN LỖI & BẢO MẬT NÂNG CAO (ERROR GUESSING & SECURITY)

#### `TC_UC06_034` - Tải lên tệp PDF bị khóa mật khẩu (Password-protected PDF)
- **Mục đích:** Kiểm tra phản ứng của bộ giải mã file khi gặp tài liệu bảo mật bằng mật khẩu.
- **Loại kiểm thử:** Negative / Security.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P2 (Medium).
...
- **Kết quả mong đợi:**
  ...
  - Không làm sập (crash) background worker của Extension.
...
#### `TC_UC06_037` - Trích xuất ứng viên đã tồn tại trong CSDL (Trùng lặp Email/SĐT)
...
#### `TC_UC06_038` - Kéo thả đồng thời nhiều file (> 1 file) vào Drag & Drop Zone
- **Mục đích:** Kiểm tra hành vi hệ thống khi người dùng kéo thả nhiều file cùng lúc.
- **Loại kiểm thử:** UI/UX / Boundary.
- **Kỹ thuật áp dụng:** Error Guessing.
- **Mức độ ưu tiên:** P3 (Low).
- **Các bước thực hiện:**
...
| - Kiểm thử chức năng (Functional / Positive): | 14 | 36.8% |
| - Kiểm thử phi chức năng, biên & ngoại lệ (Negative / Boundary / Exception): | 15 | 39.5% |
| - Kiểm thử bảo mật (Security / XSS / Malicious files): | 5 | 13.2% |
| - Kiểm thử độ tin cậy & mạng (Reliability / Network / Session): | 4 | 10.5% |
| **Phân bổ theo mức độ ưu tiên (Priority):** | | |
| - P1 (Critical / High): | 20 | 52.6% |
| - P2 (Medium): | 14 | 36.8% |
| - P3 (Low): | 4 | 10.6% |

### Kết luận đánh giá:
Bộ test case trên đã bao phủ 100% các luồng trong tài liệu đặc tả Use Case 06 của tác giả Phùng Văn Duy:
1. Bao phủ đầy đủ **Luồng chính (Web Parsing)** trên cả 3 nền tảng mục tiêu (LinkedIn, TopCV, VietnamWorks).
2. Bao phủ đầy đủ **Luồng thay thế (Upload CV File)** cho tất cả các định dạng được hỗ trợ (PDF, DOCX, Image) và thao tác kéo thả.
3. Bao phủ 100% các trường hợp ngoại lệ **`EX_01` (File quá dung lượng/sai định dạng)**, **`EX_02` (AI Timeout > 10s / Server Error)** và **`EX_03` (Website lạ không hỗ trợ)**.
4. Áp dụng chuẩn chỉ các kỹ thuật kiểm thử cốt lõi học phần CSE462: **Phân vùng tương đương (EP)**, **Phân tích giá trị biên (BVA)**, **Bảng quyết định (Decision Table)**, **Chuyển trạng thái (State Transition)** và **Đoán lỗi (Error Guessing)**.
